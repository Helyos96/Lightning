use super::draw_data::{get_rect, node_pos};
use lightning_model::data::{tree::{Node, NodeType}};
use quadtree_f32::{Item, ItemId, QuadTree};

pub struct QuadTreeHover(QuadTree);

impl QuadTreeHover {
    pub fn build(nodes: &im::HashMap<u32, Node>) -> QuadTreeHover {
        let items = nodes
        .values()
        .filter(|n| n.group.is_some() && n.class_start_index.is_none() && !n.is_ascendancy_start && !n.is_proxy && (n.name != "Medium Jewel Socket") && (n.name != "Small Jewel Socket"))
        .map(|n| {
            let (x,y) = node_pos(n);
            let (rect, _) = get_rect(n, true).unwrap();
            let scale = match n.node_type() {
                NodeType::Mastery => 1.4,
                NodeType::Notable => 2.6,
                _ => 2.5,
            };
            (
                ItemId(n.skill as usize),
                Item::Rect(quadtree_f32::Rect {
                    max_x: x + (rect.w as f32 * scale) / 2.0 + (5.0 * scale),
                    max_y: y + (rect.h as f32 * scale) / 2.0 + (5.0 * scale),
                    min_x: x - (rect.w as f32 * scale) / 2.0 - (5.0 * scale),
                    min_y: y - (rect.h as f32 * scale) / 2.0 - (5.0 * scale),
                })
            )
        });

        QuadTreeHover { 0: QuadTree::new(items) }
    }

    pub fn get_hovered_node(&self, x: f32, y: f32) -> Option<u32> {
        let overlaps = self.0.get_ids_that_overlap(&quadtree_f32::Rect {
            max_x: x + 1.0,
            max_y: y + 1.0,
            min_x: x - 1.0,
            min_y: y - 1.0,
        });

        if overlaps.is_empty() {
            return None;
        }
        Some(overlaps[0].0 as u32)
    }
}
