...
impl Default for ModEffect {
    fn default() -> Self {
        ModEffect::Stat(ModStat { stat: StatId::Strength, typ: Type::Base, amount: 0, revised_amount: None})
    }
}
...
impl Mod {
    pub fn stat(stat: StatId, typ: Type, amount: i64) -> Self {
        Self {
            effect: ModEffect::Stat(ModStat {
                stat, typ, amount, revised_amount: None
            }),
            ..Default::default()
        }
    }
    ...
    pub fn final_amount(&self) -> i64 {
        match self.effect {
            ModEffect::Stat(stat) => {
                if let Some(revised_amount) = stat.revised_amount {
                    revised_amount
                } else {
                    stat.amount
                }
    ...
    pub fn set_revised_amount(&mut self, amount: Option<i64>) {
        if let ModEffect::Stat(stat) = &mut self.effect {
            stat.revised_amount = amount;
        }
    }
